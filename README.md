### Hi there 👋

I am a software engineer with a focus on backend development.

### Tech Stack 👨🏻‍💻
- Frontend / Desktop: React, Blazor, Electron, HTML, CSS
- Backend: .NET, .NET Framework, Node.js, GraphQL, NestJS, Spring Boot, 
- Databases: MongoDB, MySQL, MSSQL, PostgreSQL, SQLite, Redis
- Languages: TS, JS, Python, Java, C#, VB.NET, C, C++
- ML / DL: PyTorch, TensorFlow, scikit-learn, pandas, matplotlib, NumPy
- Others: GitHub Actions, Jenkins, Linux, Docker, Git, Maven, Google Cloud Platform, Azure, Elasticsearch, Kibana, Jest, Webpack

<!--
**swlee052/swlee052** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.
Here are some ideas to get you started:

- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ... 
-->
